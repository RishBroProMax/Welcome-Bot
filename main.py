...
@RishBroProMax.on_message(filters.command("start"))
async def start(client: RishBroProMax, update):
    start_msg = "👋Hy {}, Iam Simple Auto Welcome Bot\n\nBot Owner Only /admin\n\nMaintained By [RishBro](https://youtube.com/@Rish_Bro/)"
    bot_username = await client.get_me()
    link = "RishBropROmAX/Welcome-Bot"
    reply_markup = InlineKeyboardMarkup(
        [             
            [
                InlineKeyboardButton
                    (
                         "🤖More Bots", url="t.me/EmoBotDeveloper"
                    ),
                InlineKeyboardButton
                    (
                         "💡Open Source", url="https://github.com/{link}" 
                    )
            ],   
            [
                InlineKeyboardButton
...
@RishBroProMax.on_message(filters.private & filters.command("admin"))
async def admin(bot: RishBroProMax, update):
    # Heroku Support
...
@RishBroProMax.on_message(filters.new_chat_members)
async def auto_welcome(bot: RishBroProMax, msg: Message):
    # from  import ID-Bot
    ...
    link_button = "t.me/EmoBotDeveloper"
